...
--(4) Write a SQL statement to know how many customer have listed their names.
 ...
 select  city as City, max(grade) as Grade from customer 
 group by city;
 ...
 select customer_id ,max(purch_amt)  as Purches_Amount from orders
 ...
 select distinct ord_date as order_Date, customer_id as Customer_Id, max(purch_amt) as Purchase_Amount from orders
 group by customer_id,ord_date;


 --(11) Write a SQL statement to find the highest purchase amount on a date '2012-08-17' for each salesman with their IkD.

 select salesman_id as Salesman_Id, max(purch_amt) as Purchase_Amount from orders
 where ord_date ='2012-08-17'
 group by salesman_id;

 select * from orders;

 --(12) Write a SQL statement to find the highest purchase amount with their ID
 --and order date, for only those customers who have highest purchase amount in a day is more than 2000.  
 select customer_id as Customer_id,ord_date as Order_Date, max(purch_amt) as Purchase_Amount from orders
 where purch_amt >2000
 group by customer_id,ord_date;

--(13) Write a SQL statement to find the highest purchase amount with their ID and
--order date, for those customers who have a higher purchase amount in a day is within the range 2000 and 6000

 select customer_id as Customer_Date,ord_date as Order_Date, max(purch_amt) as Purchase_Amount from orders
 where (purch_amt >2000 and purch_amt<6000)
 group by customer_id,ord_date;

 --(14) Write a SQL statement to find the highest purchase amount with their ID and
 --order date, for only those customers who have a higher purchase amount in a day is within 
...
--for only those customers whose ID is within the range 3002 and 3007.
 ...
 select customer_id as Customer_Id, max(purch_amt) as Purchase_Amount from orders
 ...
 --(17) Write a SQL statement to find the highest purchase amount with their ID, for
 --only those salesmen whose ID is within the range 5003 and 5008. 

  select customer_id as Customer_Id ,salesman_id as Salesman_Id, max(purch_amt) as Purchase_Amount from orders
  where (salesman_id between 5003 and 5008) 
 group by customer_id ,salesman_id;

--(18) Write a SQL statement that counts all orders for a date August 17th, 2012. 

select count(ord_date) as Order_Date from orders
where ord_date='2012-08-17';

--(19) Write a SQL statement that count the number of salesmen for whom a city is 
--specified. Note that there may be spaces or no spaces in the city column if no 
--city is specified. 
select  count(salesman_id ) as Salesman_Id from salesman
where city is not null;

--(20) Write a query that counts the number of salesmen with their order date and ID registering orders for each day.
select count(salesman_id)as Salesman_Id ,ord_date as Order_Date ,salesman_id as Salesman_Id from orders
group by  salesman_id,ord_date;
--(21) Write a SQL query to calculate the average price of all the products. 
select AVG(PRO_PRICE)  as Product_Price from item_mast;

--(22) Write a SQL query to find the number of products with a price more than or equal to Rs.350.
select count(PRO_PRICE) as Product_Price from item_mast
where PRO_PRICE >=350;
--(23) Write a SQL query to display the average price of each company's products, along with their code.
 select AVG(PRO_PRICE) as Product_Price ,PRO_COM  as Company_product from item_mast
 group by PRO_COM;
...
--(25) Write a query in SQL to find the number of employees in each department along with the department code.
 select DPT_CODE ,DPT_NAME from emp_department;